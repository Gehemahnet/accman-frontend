  ...
  <Menubar :model="navigationMenu">
    ...
    <template #end>
      <div class="flex items-center gap-2">
        ...
        <button aria-haspopup="true" aria-controls="profile-menu" @click="toggleProfileMenu">
          <Avatar image="/images/avatar/amyelsner.png" shape="circle" />
        </button>
        <Menu id="profile-menu" ref="profile-menu" :model="profileMenu" popup>
          <template #item="{item}">
            <router-link v-if="item.route" :to="{name: item.route}">{{item.label}}</router-link>
